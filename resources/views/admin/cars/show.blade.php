<div class="card bg-secondary bg-opacity-10 border-0 shadow-lg" style="overflow: hidden; border-radius: 24px;">
    <div class="row g-0">
        <div class="col-lg-7 position-relative" style="min-height: 450px;">
            @if ($car->image)
                <img src="{{ asset('storage/' . $car->image) }}" class="img-fluid w-100 h-100" style="object-fit: cover;"
                    alt="{{ $car->model }}">
            @else
        ...
        <div class="col-lg-5 p-5 d-flex flex-column">
            <div class="mb-4">
                <span class="badge bg-primary mb-2">{{ $car->category->name ?? 'Armada' }}</span>
                <h2 class="fw-bold text-white mb-1">{{ $car->model }}</h2>
                <p class="text-secondary small"><i class="bi bi-hash me-1"></i>{{ $car->plate_number }}</p>
            </div>
            ...
            <div class="row g-3 mb-4">
                <div class="col-6">
                    <div class="p-3 bg-dark rounded-4 text-center border border-secondary border-opacity-10">
                        ...
                        <strong class="text-white">{{ $car->capacity }} Kursi</strong>
                    </div>
                </div>
                <div class="col-6">
                    <div class="p-3 bg-dark rounded-4 text-center border border-secondary border-opacity-10">
                        ...
                        <strong class="text-white">{{ $car->transmission }}</strong>
                    </div>
                </div>
                <div class="col-6">
                    <div class="p-3 bg-dark rounded-4 text-center border border-secondary border-opacity-10">
                ...
                <div class="col-6">
                    <div class="p-3 bg-dark rounded-4 text-center border border-secondary border-opacity-10">
                        ...
                        <strong class="text-white">{{ $car->color ?? '-' }}</strong>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-auto pt-4 border-top border-secondary border-opacity-10">
                <div class="d-flex justify-content-between align-items-end">
                    ...
                    <a href="{{ route('admin.cars.edit', $car->id) }}" class="btn btn-primary btn-sm rounded-3 px-2">
